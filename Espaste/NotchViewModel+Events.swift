    ...
    func setupCancellables() {
        ...
        events.mouseLocation
            ...
            .sink { [weak self] mouseLocation in
                ...
                if status == .closed, aboutToOpen { notchOpen(.click) }
                if status == .opened, !notchOpenedRect.contains(mouseLocation) { notchClose() }
            }
            .store(in: &cancellables)
